                    ...
                    ForEach(0..<50) { index in
                        GeometryReader { geo in
                            Text("Row #\(index)")
                                ...
                                .rotation3DEffect(
                                    .degrees(Double(geo.frame(in: .global).minY - fullView.size.height / 2) / 5), axis: (x: 0, y: 1, z: 0)
                                )
                        }
                        .frame(height: 40)
                    }
                        ...
                        ForEach(0..<50) { index in
                            GeometryReader { geo in
                                Rectangle()
                                    ...
                                    .frame(height: 150)
                                    .rotation3DEffect(
                                        .degrees(-Double(geo.frame(in: .global).midX - fullView.size.width / 2) / 10), axis: (x: 0, y: 1, z: 0)
                                    )
                            }
                            .frame(width: 150)
                        }
